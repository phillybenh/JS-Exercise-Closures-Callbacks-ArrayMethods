...
// console.log(runners); just checking
...
// Bringing over the data so it's easier to console.log as I go
const runners = [
...
function firstNamesAllCaps(runners) {
  ...
  const capNames = runners.map((item) => item.first_name.toUpperCase())
  return capNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const shirtSize = runners.filter((item) => {
    return item.shirt_size === tShirtSize;
  })
  // const shirtSize = runners.shirt_size.filer((item) => {
  //   if (item === tShirtSize) {
  //     return true;
  //   }
  //   else {
...
function tallyUpDonations(runners) {
  const totalDonations = runners.reduce((accum, item) => {
